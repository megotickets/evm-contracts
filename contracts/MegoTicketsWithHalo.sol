...
contract MegoTicketsWithHalo is ERC721, Ownable, Pausable {
    ...
    mapping(address => address) public _linkedToOwner;
    ...
    function tokensOfLinkedAddress(address _linked)
        external
        view
        returns (uint256[] memory ownerTokens)
    {
        address _owner = _linkedToOwner[_linked];
        uint256 tokenCount = balanceOf(_owner);
        if (tokenCount == 0) {
            // Return an empty array
            return new uint256[](0);
    ...
    function linkAddress(address _linked) external {
        require(_linkedToOwner[_linked] == address(0), "Address already linked");
        _linkedToOwner[_linked] = msg.sender;
    }

    function unlinkAddress(address _linked) external {
        require(_linkedToOwner[_linked] == msg.sender, "Can't manage this halo");
        _linkedToOwner[_linked] = address(0);
    }
