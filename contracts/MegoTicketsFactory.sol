...
contract MegoTicketsFactory {
    ...
    event MegoTicketsPublicDeployed(address indexed deployer, address deployedAddress);
    
    // Function to create a new MegoTicketsPublic contract
    function createMegoTicketsPublic(string memory _name, string memory _ticker) external returns (address) {
    ...
    function getDeployedContracts(address deployer) external view returns (address[] memory) {
